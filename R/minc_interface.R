...
mincGetVoxel <- function(filenames, v1, v2=NULL, v3=NULL) {
  ...
  output <- .C("get_voxel_from_files",
               ...
               as.integer(v2),
               as.integer(v3),
               o=double(length=num.files), PACKAGE="rminc2")$o
...
mincConvertVoxelToWorld <- function(filename, v1, v2, v3) {
  output <- .C("convert_voxel_to_world",
               ...
               as.double(v2),
               as.double(v3),
               o=double(length=3), PACKAGE="rminc2")$o
  return(output)
}
...
mincGetVolume <- function(filename) {
  ...
  output <- .C("get_hyperslab",
               ...
               as.integer(start),
               as.integer(sizes),
               hs=double(total.size), PACKAGE="rminc2")$hs
...
mincWriteVolume.default <- function(buffer, output.filename, like.filename,
  ...
  output <- .C("write_minc2_volume",
               ...
               as.double(b.max),
               as.double(b.min),
               as.double(buffer), PACKAGE="rminc2")
}
...
minc.dimensions.sizes <- function(filename) {
  sizes <- .C("get_volume_sizes",
              as.character(filename),
              sizes = integer(3), PACKAGE="rminc2")$sizes
  return(sizes)
}
...
#                (count[3] - start[3])
  ...
  output <- .C("get_hyperslab",
               as.character(filename),
               as.integer(start),
               as.integer(count), hs=buffer, PACKAGE="rminc2")$hs
  return(output)
}
...
###                   as.character(mask),
###                   NULL, NULL,
###                   as.character(method), PACKAGE="rminc2")
  result <- .Call("per_voxel_anova",
                  as.character(filenames),
                  as.matrix(mmatrix),
                  attr(mmatrix, "assign"),
                  as.double(! is.null(mask)),
                  as.character(mask), PACKAGE="rminc2")
...
mincApply <- function(filenames, function.string, mask=NULL) {
  ...
  results <- .Call("minc2_model",
                   ...
                   as.character("eval"), PACKAGE="rminc2");
...
vertexLm <- function(formula, data, subset=NULL) {
  ...
  result <- .Call("vertex_lm_loop", data.matrix, mmatrix, PACKAGE="rminc2");
