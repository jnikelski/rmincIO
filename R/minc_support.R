...
rminc2.isReadable <- function(filenames) {
  rValue <- TRUE
  READ_PERMISSION <- 4
...
rminc2.isMinc <- function(filename) {
	rValue <- FALSE
	if ( !file.exists(filename) ) { return(rValue) }
	if ( rminc2.isMinc1(filename) ) rValue <- TRUE
	if ( rminc2.isMinc2(filename) ) rValue <- TRUE
	#
	return(rValue)
}
...
rminc2.isMinc1 <- function(filename) {
	rValue <- FALSE
...
rminc2.isMinc2 <- function(filename) {
	rValue <- FALSE
	sysCmd <- paste("file", filename)
#	print(sysCmd)
	rtnString <- system(sysCmd, intern=TRUE)
...
rminc2.asMinc2 <- function(filename, keepName=TRUE) {
	
	# is it already minc2? Just return the input filename.
	if ( rminc2.isMinc2(filename) ) return(filename)
...
rminc2.convertVoxelToWorld <- function(filename, voxCoords) {
	#
	
	if ( R_DEBUG_rminc2 ) cat(sprintf(">>rminc2.convertVoxelToWorld\n"))
	...
	output <- .Call("convert_voxel_to_world_mincIO",
               ...
               as.double(voxCoords), PACKAGE="rminc2")
...
rminc2.convertWorldToVoxel <- function(filename, worldCoords) {
	#
	# dunno why, but the voxel coordinates are passed as doubles
	if ( R_DEBUG_rminc2 ) cat(sprintf(">>rminc2.convertWorldToVoxel\n"))

	output <- .Call("convert_world_to_voxel_mincIO",
               as.character(filename),
               as.double(worldCoords), PACKAGE="rminc2")
	if ( R_DEBUG_rminc2 ) {
...
rminc2.getDataClasses <- function() {
	# =============================================================================
...
rminc2.checkForExternalProgram <- function(program, test_string, prog_options="") {
	# =============================================================================
	...
	#	result <- rminc2.checkForExternalProgram(program, test_string, progOptions)
	#	if ( !result ) { ... }
	#
	# Note: Nothing of note, really.
	#
...
rminc2.readLinearXfmFile <- function(xfmFilename) {
	# =============================================================================
	...
	tmpfile <- tempfile("rminc2.readLinearXfmFile")
